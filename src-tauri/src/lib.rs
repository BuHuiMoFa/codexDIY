...
fn decode_xml_entities(s: &str) -> String {
    ...
    while let Some(entity_start) = rest.find('&') {
        ...
        let Some(entity_end) = candidate.find(';') else {
            out.push_str(candidate);
            return out;
        };

        let entity = &candidate[..=entity_end];
...
fn archive_entry_path(raw: &str) -> String {
    let normalized = raw.replace('\\', "/").trim().trim_end_matches('/').to_string();
...
fn encode_archive_preview(payload: ArchivePreviewPayload) -> Result<String, String> {
    serde_json::to_string(&payload)
        .map(|json| format!("{}{}", ARCHIVE_PREVIEW_PREFIX, json))
        .map_err(|e| format!("Cannot serialize archive preview: {}", e))
}

fn preview_zip_archive(path: &str) -> Result<String, String> {
    let file = std::fs::File::open(path).map_err(|e| format!("Cannot read archive: {}", e))?;
    let mut archive = zip::ZipArchive::new(file).map_err(|e| format!("Cannot open zip: {}", e))?;
    let total_entries = archive.len();
    let mut entries = Vec::new();

    for index in 0..total_entries {
        let file = archive.by_index(index).map_err(|e| format!("Cannot read zip entry: {}", e))?;
        if entries.len() >= MAX_ARCHIVE_PREVIEW_ENTRIES {
            break;
        }
        entries.push(ArchivePreviewEntry {
            path: archive_entry_path(file.name()),
            kind: if file.is_dir() { "dir".to_string() } else { "file".to_string() },
            size: Some(file.size()),
            compressed_size: Some(file.compressed_size()),
        });
    }

    encode_archive_preview(ArchivePreviewPayload {
        kind: "archive",
...
fn preview_tar_archive_reader<R: std::io::Read>(reader: R, format: &str) -> Result<String, String> {
    let mut archive = tar::Archive::new(reader);
    let mut entries = Vec::new();
    let mut total_entries = 0usize;

    for entry_result in archive.entries().map_err(|e| format!("Cannot open tar archive: {}", e))? {
        let entry = entry_result.map_err(|e| format!("Cannot read tar entry: {}", e))?;
        total_entries += 1;
        if entries.len() >= MAX_ARCHIVE_PREVIEW_ENTRIES {
            continue;
        }
        ...
        entries.push(ArchivePreviewEntry {
            path: archive_entry_path(&path),
            kind,
            size: Some(entry.header().size().unwrap_or(0)),
...
fn preview_tar_archive(path: &str) -> Result<String, String> {
    let file = std::fs::File::open(path).map_err(|e| format!("Cannot read archive: {}", e))?;
    preview_tar_archive_reader(file, "tar")
}

fn preview_targz_archive(path: &str) -> Result<String, String> {
    let file = std::fs::File::open(path).map_err(|e| format!("Cannot read archive: {}", e))?;
    let decoder = flate2::read::GzDecoder::new(file);
    preview_tar_archive_reader(decoder, "tar.gz")
}

fn preview_gzip_stream(path: &str) -> Result<String, String> {
    let file_name = std::path::Path::new(path)
        .file_name()
        .and_then(|value| value.to_str())
        .unwrap_or("archive.gz");
    let logical_name = file_name.strip_suffix(".gz").unwrap_or(file_name).to_string();
    ...
    encode_archive_preview(ArchivePreviewPayload {
        ...
        total_entries: 1,
        truncated: false,
        note: Some("This gzip file contains a single compressed stream.".to_string()),
    })
}

#[cfg(target_os = "windows")]
fn find_7zip_executable() -> std::path::PathBuf {
    let candidates = [
        std::env::var_os("ProgramFiles")
            .map(|base| std::path::PathBuf::from(base).join("7-Zip").join("7z.exe")),
...
#[cfg(target_os = "windows")]
fn preview_via_7zip(path: &str, format: &str) -> Result<String, String> {
    let exe = find_7zip_executable();
    let mut cmd = std::process::Command::new(exe);
    cmd.args(["l", "-slt", "-sccUTF-8", path]);
    cmd.creation_flags(0x08000000);
    let output = cmd
        .output()
        .map_err(|e| format!("Cannot launch 7-Zip for preview: {}", e))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
        let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
        let detail = if !stderr.is_empty() { stderr } else { stdout };
        return Err(format!("Cannot inspect {} archive: {}", format, detail));
    }
    ...
    let mut flush_entry = |entry: &mut BTreeMap<String, String>, out: &mut Vec<ArchivePreviewEntry>, total: &mut usize| {
        if entry.is_empty() {
            return;
        }
        if let Some(path_value) = entry.get("Path").cloned() {
            *total += 1;
            if out.len() < MAX_ARCHIVE_PREVIEW_ENTRIES {
                let attributes = entry.get("Attributes").cloned().unwrap_or_default();
                let kind = if attributes.contains('D') { "dir" } else { "file" }.to_string();
                let size = entry.get("Size").and_then(|v| v.parse::<u64>().ok());
                let packed_size = entry.get("Packed Size").and_then(|v| v.parse::<u64>().ok());
                out.push(ArchivePreviewEntry {
                    path: archive_entry_path(&path_value),
                    kind,
                    size,
    ...
    for line in text.lines() {
        ...
        if trimmed.is_empty() {
            flush_entry(&mut current, &mut entries, &mut total_entries);
            continue;
        }
    ...
    encode_archive_preview(ArchivePreviewPayload {
        ...
        total_entries,
        truncated: total_entries > MAX_ARCHIVE_PREVIEW_ENTRIES,
        note: Some("Preview generated via local 7-Zip archive listing.".to_string()),
    })
}

#[cfg(not(target_os = "windows"))]
fn preview_via_7zip(_path: &str, format: &str) -> Result<String, String> {
    Err(format!("{} preview is currently only enabled on Windows builds", format))
}
...
fn extract_tag_text(xml: &str, tag: &str) -> String {
    ...
    while let Some(open_idx) = xml[search_from..].find(&open_prefix) {
        let start = search_from + open_idx;
        let after_open = match xml[start..].find('>') {
            Some(idx) => start + idx + 1,
            None => break,
        };
        let Some(close_idx_rel) = xml[after_open..].find(&close_tag) else { break; };
        let close = after_open + close_idx_rel;
        let piece = xml[after_open..close].trim();
        if !piece.is_empty() {
            if !out.is_empty() {
                out.push('\n');
            }
            out.push_str(&decode_xml_entities(piece));
        }
...
fn extract_all_tag_texts(xml: &str, tag: &str) -> Vec<String> {
    ...
    while let Some(open_idx) = xml[search_from..].find(&open_prefix) {
        ...
        let piece = xml[after_open..close].trim();
        if !piece.is_empty() {
            out.push(decode_xml_entities(piece));
        }
        search_from = close + close_tag.len();
    }
...
fn extract_docx_text(xml: &str) -> String {
    ...
    while search_from < xml.len() {
        ...
        for token in ["<w:t", "<w:tab/>", "<w:br/>", "</w:p>"] {
            if let Some(idx) = remaining.find(token) {
                match next {
                    Some((_, best_idx)) if idx >= best_idx => {}
                    _ => next = Some((token, idx)),
                }
            }
        }

        let Some((token, idx)) = next else { break; };
        search_from += idx;

        match token {
            "<w:t" => {
                let after_open = match xml[search_from..].find('>') {
                    Some(idx) => search_from + idx + 1,
                    None => break,
                };
                let Some(close_rel) = xml[after_open..].find("</w:t>") else { break; };
            ...
            "<w:tab/>" => {
                out.push('\t');
                search_from += "<w:tab/>".len();
            }
            "<w:br/>" => {
                out.push('\n');
                search_from += "<w:br/>".len();
            }
            "</w:p>" => {
                if !out.ends_with('\n') {
                    out.push('\n');
...
const ARCHIVE_PREVIEW_PREFIX: &str = "__TOKENICODE_ARCHIVE_PREVIEW__";
...
#[derive(Serialize)]
struct XlsxPreviewSheet {
    name: String,
    rows: Vec<Vec<XlsxPreviewCell>>,
    total_rows: usize,
    total_columns: usize,
    truncated_rows: bool,
...
#[derive(Serialize)]
struct XlsxPreviewPayload {
    kind: String,
    sheets: Vec<XlsxPreviewSheet>,
    total_rows: usize,
}

struct XlsxImageBudget {
    remaining_images: usize,
    remaining_bytes: usize,
}

fn read_xmlish_string(reader: &mut impl std::io::Read, error_prefix: &str) -> Result<String, String> {
...
fn read_zip_entry_bytes(
    ...
    let mut entry = zip
        .by_name(path)
        .map_err(|e| format!("{} {}: {}", error_prefix, path, e))?;
    let mut bytes = Vec::new();
    entry
        .read_to_end(&mut bytes)
        .map_err(|e| format!("{} {}: {}", error_prefix, path, e))?;
...
fn derive_zip_rels_path(path: &str) -> String {
    ...
    if parts.is_empty() {
        ...
        format!("{}/_rels/{}.rels", parts.join("/"), file_name)
    }
}

fn normalize_zip_target(base_path: &str, target: &str) -> String {
    ...
    let mut base_parts: Vec<&str> = normalized_base.split('/').collect();
...
fn extract_xml_blocks_multi<'a>(xml: &'a str, patterns: &[(&str, &str)]) -> Vec<&'a str> {
    let mut spans = Vec::new();
    for (open_prefix, close_tag) in patterns {
        let mut search_from = 0;
        while let Some(open_rel) = xml[search_from..].find(open_prefix) {
            let start = search_from + open_rel;
            let Some(close_rel) = xml[start..].find(close_tag) else { break; };
...
fn extract_first_xml_block_multi<'a>(
    xml: &'a str,
    patterns: &[(&str, &str)],
) -> Option<&'a str> {
    extract_xml_blocks_multi(xml, patterns).into_iter().next()
}

fn extract_first_non_empty_tag_text(xml: &str, tags: &[&str]) -> Option<String> {
    for tag in tags {
        let raw = extract_tag_text(xml, tag);
...
fn extract_xml_attr(tag_xml: &str, attr: &str) -> Option<String> {
    for quote in ['"', '\''] {
        let needle = format!("{}={}", attr, quote);
        if let Some(start) = tag_xml.find(&needle) {
            let value_start = start + needle.len();
            let rest = &tag_xml[value_start..];
...
fn normalize_xlsx_sheet_target(target: &str) -> String {
    let normalized = target.replace('\\', "/");
    if normalized.starts_with("xl/") {
        normalized
    } else {
        format!("xl/{}", normalized.trim_start_matches("./").trim_start_matches('/'))
    }
}

fn parse_cell_ref_col_index(cell_ref: &str) -> Option<usize> {
    let mut col = 0usize;
...
fn xlsx_preview_image_mime(path: &str) -> &'static str {
...
fn extract_relationship_targets(
    zip: &mut zip::ZipArchive<std::fs::File>,
    owner_path: &str,
) -> Result<HashMap<String, String>, String> {
    let rels_path = derive_zip_rels_path(owner_path);
    let Ok(mut rels_file) = zip.by_name(&rels_path) else {
        return Ok(HashMap::new());
    };

    let rels_xml = read_xmlish_string(&mut rels_file, "Cannot read xlsx relationships")?;
    let mut targets = HashMap::new();
    let mut search_from = 0;
    while let Some(rel_open) = rels_xml[search_from..].find("<Relationship") {
        let start = search_from + rel_open;
        let Some(tag_end_rel) = rels_xml[start..].find('>') else { break; };
        let tag_end = start + tag_end_rel + 1;
        let tag_xml = &rels_xml[start..tag_end];
        if let (Some(id), Some(target)) = (
            extract_xml_attr(tag_xml, "Id"),
            extract_xml_attr(tag_xml, "Target"),
        ) {
            targets.insert(id, normalize_zip_target(owner_path, &target));
        }
        search_from = tag_end;
    }
...
fn load_xlsx_preview_image(
    zip: &mut zip::ZipArchive<std::fs::File>,
    media_path: &str,
    image_budget: &mut XlsxImageBudget,
) -> Option<String> {
    if image_budget.remaining_images == 0 || image_budget.remaining_bytes == 0 {
        return None;
    }

    let bytes = read_zip_entry_bytes(zip, media_path, "Cannot read xlsx image").ok()?;
    if bytes.is_empty() || bytes.len() > MAX_XLSX_PREVIEW_IMAGE_BYTES || bytes.len() > image_budget.remaining_bytes {
        return None;
    }

    image_budget.remaining_images = image_budget.remaining_images.saturating_sub(1);
    image_budget.remaining_bytes = image_budget.remaining_bytes.saturating_sub(bytes.len());

    let mime = xlsx_preview_image_mime(media_path);
    let encoded = base64::engine::general_purpose::STANDARD.encode(&bytes);
    Some(format!("data:{};base64,{}", mime, encoded))
}

fn extract_xlsx_sheet_images(
    zip: &mut zip::ZipArchive<std::fs::File>,
    sheet_path: &str,
    sheet_xml: &str,
    image_budget: &mut XlsxImageBudget,
) -> Result<HashMap<(usize, usize), Vec<String>>, String> {
    let sheet_rels = extract_relationship_targets(zip, sheet_path)?;
    let mut drawing_paths = Vec::new();
    let mut search_from = 0;

    while let Some(draw_open) = sheet_xml[search_from..].find("<drawing") {
        let start = search_from + draw_open;
        let Some(tag_end_rel) = sheet_xml[start..].find('>') else { break; };
        let tag_end = start + tag_end_rel + 1;
        let tag_xml = &sheet_xml[start..tag_end];
        if let Some(rel_id) = extract_xml_attr(tag_xml, "r:id") {
            if let Some(target) = sheet_rels.get(&rel_id) {
                drawing_paths.push(target.clone());
            }
        }
        search_from = tag_end;
    }

    let mut images_by_cell: HashMap<(usize, usize), Vec<String>> = HashMap::new();

    for drawing_path in drawing_paths {
        let drawing_xml = match zip.by_name(&drawing_path) {
            Ok(mut entry) => read_xmlish_string(&mut entry, "Cannot read xlsx drawing")?,
            Err(_) => continue,
        };
        let drawing_rels = extract_relationship_targets(zip, &drawing_path)?;
        let anchors = extract_xml_blocks_multi(
            &drawing_xml,
            &[
        ...
        for anchor_xml in anchors {
            let Some(from_xml) = extract_first_xml_block_multi(
                anchor_xml,
                &[("<xdr:from", "</xdr:from>"), ("<from", "</from>")],
            ) else {
                continue;
            };
            let row = extract_first_non_empty_tag_text(from_xml, &["xdr:row", "row"])
                .and_then(|raw| raw.trim().parse::<usize>().ok());
            let col = extract_first_non_empty_tag_text(from_xml, &["xdr:col", "col"])
                .and_then(|raw| raw.trim().parse::<usize>().ok());
            let (Some(row), Some(col)) = (row, col) else {
                continue;
            };

            let mut anchor_search = 0;
            while let Some(blip_open) = anchor_xml[anchor_search..].find("<a:blip") {
                let start = anchor_search + blip_open;
                let Some(tag_end_rel) = anchor_xml[start..].find('>') else { break; };
                let tag_end = start + tag_end_rel + 1;
                let tag_xml = &anchor_xml[start..tag_end];
                if let Some(embed_id) = extract_xml_attr(tag_xml, "r:embed") {
                    if let Some(media_path) = drawing_rels.get(&embed_id) {
                        if let Some(image_data_url) = load_xlsx_preview_image(zip, media_path, image_budget) {
                            images_by_cell
                                .entry((row, col))
                                .or_default()
                                .push(image_data_url);
...
fn extract_xlsx_shared_strings(zip: &mut zip::ZipArchive<std::fs::File>) -> Result<Vec<String>, String> {
    let mut shared_strings = Vec::new();
    let Ok(mut shared) = zip.by_name("xl/sharedStrings.xml") else {
        return Ok(shared_strings);
    };

    let xml = read_xmlish_string(&mut shared, "Cannot read xlsx shared strings")?;
    let mut search_from = 0;
    while let Some(si_open) = xml[search_from..].find("<si") {
        let start = search_from + si_open;
        let after_open = match xml[start..].find('>') {
            Some(idx) => start + idx + 1,
            None => break,
        };
        let Some(si_close_rel) = xml[after_open..].find("</si>") else { break; };
        let close = after_open + si_close_rel;
        let si_xml = &xml[after_open..close];
        let text = extract_all_tag_texts(si_xml, "t").join("");
        shared_strings.push(text);
        search_from = close + "</si>".len();
    }

    Ok(shared_strings)
}

fn extract_xlsx_sheet_order(zip: &mut zip::ZipArchive<std::fs::File>) -> Result<Vec<(String, String)>, String> {
    let mut rels_by_id = HashMap::new();
    if let Ok(mut rels_file) = zip.by_name("xl/_rels/workbook.xml.rels") {
        let rels_xml = read_xmlish_string(&mut rels_file, "Cannot read xlsx workbook rels")?;
        let mut search_from = 0;
        while let Some(rel_open) = rels_xml[search_from..].find("<Relationship") {
            let start = search_from + rel_open;
            let Some(tag_end_rel) = rels_xml[start..].find('>') else { break; };
            ...
            if let (Some(id), Some(target)) = (
                ...
                rels_by_id.insert(id, normalize_xlsx_sheet_target(&target));
            }
            search_from = tag_end;
        }
    }

    let Ok(mut workbook_file) = zip.by_name("xl/workbook.xml") else {
        return Ok(Vec::new());
    };

    let workbook_xml = read_xmlish_string(&mut workbook_file, "Cannot read xlsx workbook")?;
    let mut sheets = Vec::new();
    let mut search_from = 0;
    while let Some(sheet_open) = workbook_xml[search_from..].find("<sheet") {
        let start = search_from + sheet_open;
        let Some(tag_end_rel) = workbook_xml[start..].find('>') else { break; };
        let tag_end = start + tag_end_rel + 1;
        let tag_xml = &workbook_xml[start..tag_end];
        if tag_xml.starts_with("<sheets") {
            search_from = tag_end;
            continue;
        }
        if let (Some(name), Some(rel_id)) = (
            extract_xml_attr(tag_xml, "name"),
            extract_xml_attr(tag_xml, "r:id"),
        ) {
            if let Some(target) = rels_by_id.get(&rel_id) {
...
fn extract_xlsx_cell_value(cell_xml: &str, cell_tag: &str, shared_strings: &[String]) -> String {
    match extract_xml_attr(cell_tag, "t").as_deref() {
        Some("s") => extract_tag_text(cell_xml, "v")
            .trim()
            .parse::<usize>()
            .ok()
            .and_then(|idx| shared_strings.get(idx).cloned())
            .unwrap_or_default(),
        Some("inlineStr") => extract_all_tag_texts(cell_xml, "t").join(""),
        Some("b") => match extract_tag_text(cell_xml, "v").trim() {
            "1" => "TRUE".to_string(),
            "0" => "FALSE".to_string(),
            other => other.to_string(),
        },
        _ => extract_tag_text(cell_xml, "v"),
    }
}

fn extract_xlsx_sheet_preview(
    name: String,
    xml: &str,
    shared_strings: &[String],
    images_by_cell: &HashMap<(usize, usize), Vec<String>>,
) -> XlsxPreviewSheet {
    let mut rows = Vec::new();
    let mut total_columns = 0usize;
    let mut row_texts: BTreeMap<usize, BTreeMap<usize, String>> = BTreeMap::new();
    let mut sequential_row_idx = 0usize;
    let mut truncated_columns = false;
    let mut search_from = 0;

    while let Some(row_open) = xml[search_from..].find("<row") {
        let row_start = search_from + row_open;
        let row_tag_end = match xml[row_start..].find('>') {
            Some(idx) => row_start + idx + 1,
            None => break,
        };
        let row_tag = &xml[row_start..row_tag_end];
        let Some(row_close_rel) = xml[row_tag_end..].find("</row>") else { break; };
        let row_close = row_tag_end + row_close_rel;
        let row_xml = &xml[row_tag_end..row_close];
        let row_idx = extract_xml_attr(row_tag, "r")
            .and_then(|value| value.parse::<usize>().ok())
            .map(|value| value.saturating_sub(1))
            .unwrap_or(sequential_row_idx);
        sequential_row_idx = row_idx.saturating_add(1);
        let mut row_cells: BTreeMap<usize, String> = BTreeMap::new();
        let mut row_cursor = 0usize;
        let mut next_col_idx = 0usize;

        while let Some(cell_open) = row_xml[row_cursor..].find("<c") {
            let cell_start = row_cursor + cell_open;
            let Some(tag_end_rel) = row_xml[cell_start..].find('>') else { break; };
            let tag_end = cell_start + tag_end_rel + 1;
            let cell_tag = &row_xml[cell_start..tag_end];
            let is_self_closing = cell_tag.trim_end().ends_with("/>");
            let cell_end = if is_self_closing {
                tag_end
            } else if let Some(close_rel) = row_xml[tag_end..].find("</c>") {
                tag_end + close_rel + "</c>".len()
            } else {
                row_xml.len()
            };
            let cell_xml = &row_xml[cell_start..cell_end];
            let col_idx = extract_xml_attr(cell_tag, "r")
                .as_deref()
                .and_then(parse_cell_ref_col_index)
                .unwrap_or(next_col_idx);
            next_col_idx = col_idx.saturating_add(1);
            total_columns = total_columns.max(col_idx.saturating_add(1));

            let value = extract_xlsx_cell_value(cell_xml, cell_tag, shared_strings);
            if col_idx >= MAX_XLSX_PREVIEW_COLS && !value.is_empty() {
                truncated_columns = true;
            } else {
                row_cells.insert(col_idx, value);
            }
        ...
        if !row_cells.is_empty() {
            row_texts.insert(row_idx, row_cells);
        }

        search_from = row_close + "</row>".len();
    }

    for &(row_idx, col_idx) in images_by_cell.keys() {
        total_columns = total_columns.max(col_idx.saturating_add(1));
        if col_idx >= MAX_XLSX_PREVIEW_COLS {
            truncated_columns = true;
        }
        row_texts.entry(row_idx).or_default();
    }

    let row_indices: BTreeSet<usize> = row_texts.keys().copied().collect();
    let total_rows = row_indices.len();
    let truncated_rows = total_rows > MAX_XLSX_PREVIEW_ROWS;
    let preview_columns = total_columns.min(MAX_XLSX_PREVIEW_COLS);
    if total_columns > MAX_XLSX_PREVIEW_COLS {
        truncated_columns = true;
    }

    for row_idx in row_indices.into_iter().take(MAX_XLSX_PREVIEW_ROWS) {
        let mut preview_row = vec![XlsxPreviewCell::default(); preview_columns];

        if let Some(text_cells) = row_texts.get(&row_idx) {
            for (&col_idx, value) in text_cells {
                if col_idx < preview_columns {
                    preview_row[col_idx].text = value.clone();
                }
            }
        }

        for col_idx in 0..preview_columns {
            if let Some(images) = images_by_cell.get(&(row_idx, col_idx)) {
                preview_row[col_idx].images = images.clone();
            }
        }
...
fn extract_xlsx_text(zip: &mut zip::ZipArchive<std::fs::File>) -> Result<String, String> {
    let shared_strings = extract_xlsx_shared_strings(zip)?;
    let ordered_sheets = extract_xlsx_sheet_order(zip)?;
    let mut sheets = Vec::new();
    let mut used_paths = std::collections::HashSet::new();
    let mut image_budget = XlsxImageBudget {
        remaining_images: MAX_XLSX_PREVIEW_IMAGES,
        remaining_bytes: MAX_XLSX_PREVIEW_TOTAL_IMAGE_BYTES,
    };

    for (path, name) in ordered_sheets {
        let Ok(mut entry) = zip.by_name(&path) else {
            continue;
        };
        let xml = read_xmlish_string(&mut entry, "Cannot read xlsx sheet")?;
        drop(entry);
        let images_by_cell = extract_xlsx_sheet_images(zip, &path, &xml, &mut image_budget)?;
        used_paths.insert(path.clone());
        sheets.push(extract_xlsx_sheet_preview(name, &xml, &shared_strings, &images_by_cell));
    }

    let mut fallback_sheet_paths = Vec::new();
    for i in 0..zip.len() {
        let entry = zip
            .by_index(i)
            .map_err(|e| format!("Cannot read xlsx entry: {}", e))?;
        let name = entry.name().replace('\\', "/");
        if name.starts_with("xl/worksheets/") && name.ends_with(".xml") && !used_paths.contains(&name) {
            fallback_sheet_paths.push(name);
        }
    }
    fallback_sheet_paths.sort();

    for path in fallback_sheet_paths {
        let mut entry = zip
            .by_name(&path)
            .map_err(|e| format!("Cannot read xlsx sheet {}: {}", path, e))?;
        let xml = read_xmlish_string(&mut entry, "Cannot read xlsx sheet")?;
        drop(entry);
        let images_by_cell = extract_xlsx_sheet_images(zip, &path, &xml, &mut image_budget)?;
        let fallback_name = std::path::Path::new(&path)
            ...
            .unwrap_or("Sheet")
            .to_string();
        sheets.push(extract_xlsx_sheet_preview(
            fallback_name,
            &xml,
            &shared_strings,
            &images_by_cell,
    ...
    let payload = XlsxPreviewPayload {
        kind: "xlsx".to_string(),
        total_rows: sheets.iter().map(|sheet| sheet.total_rows).sum(),
        sheets,
    };
    let json = serde_json::to_string(&payload)
        .map_err(|e| format!("Cannot serialize xlsx preview: {}", e))?;
    Ok(format!("{}{}", XLSX_PREVIEW_PREFIX, json))
}

fn extract_pptx_text(zip: &mut zip::ZipArchive<std::fs::File>) -> Result<String, String> {
    use std::io::Read;
    ...
    for i in 0..zip.len() {
        let mut entry = zip
            .by_index(i)
            .map_err(|e| format!("Cannot read pptx entry: {}", e))?;
        ...
        let text = extract_all_tag_texts(&xml, "a:t").join("");
    ...
    let mut out = String::new();
    for (idx, (label, text)) in slides.iter().enumerate() {
        if idx > 0 {
            out.push_str("\n\n");
        }
        out.push_str(&format!("# {}\n\n", label));
...
struct BundledRuntimePaths {
    cli_bin_dir: Option<std::path::PathBuf>,
    git_root_dir: Option<std::path::PathBuf>,
}

static BUNDLED_RUNTIME_PATHS: OnceLock<BundledRuntimePaths> = OnceLock::new();
...
fn bundled_runtime_rel_dir() -> &'static str {
    #[cfg(target_os = "windows")]
    {
    ...
    #[cfg(target_os = "macos")]
    {
        "bundled-runtime/macos"
    }
    #[cfg(all(not(target_os = "windows"), not(target_os = "macos")))]
    {
        "bundled-runtime/linux"
    }
}

fn detect_bundled_runtime_paths(app: &AppHandle) -> BundledRuntimePaths {
    let Ok(resource_dir) = app.path().resource_dir() else {
        return BundledRuntimePaths::default();
    };

    let root = resource_dir.join(bundled_runtime_rel_dir());
    let mut paths = BundledRuntimePaths::default();

    let bundled_cli_dir = root.join("npm-global");
    #[cfg(target_os = "windows")]
    let bundled_cli_entry = bundled_cli_dir.join("claude.cmd");
    #[cfg(not(target_os = "windows"))]
    let bundled_cli_entry = bundled_cli_dir.join("bin").join("claude");
    if bundled_cli_entry.exists() {
        paths.cli_bin_dir = Some(bundled_cli_dir);
    }

    #[cfg(target_os = "windows")]
    {
        let bundled_git_dir = root.join("git");
        if bundled_git_dir.join("bin").join("bash.exe").exists() {
            paths.git_root_dir = Some(bundled_git_dir);
        }
    }
...
fn init_bundled_runtime_paths(app: &AppHandle) {
    let paths = detect_bundled_runtime_paths(app);
    let _ = BUNDLED_RUNTIME_PATHS.set(paths.clone());

    if paths.cli_bin_dir.is_some() || paths.git_root_dir.is_some() {
        eprintln!(
            "[bundled-runtime] detected cli={:?} git={:?}",
            paths.cli_bin_dir, paths.git_root_dir
        );
    }
}

pub(crate) fn bundled_cli_bin_dir() -> Option<std::path::PathBuf> {
    BUNDLED_RUNTIME_PATHS
        .get()
        .and_then(|paths| paths.cli_bin_dir.clone())
}

#[cfg(target_os = "windows")]
pub(crate) fn bundled_git_root_dir() -> Option<std::path::PathBuf> {
    BUNDLED_RUNTIME_PATHS
        .get()
        .and_then(|paths| paths.git_root_dir.clone())
}
...
pub(crate) fn find_git_bash() -> Option<String> {
    ...
    // 1.5 Check installer-bundled PortableGit next
    if let Some(bundled_root) = bundled_git_root_dir() {
        let bundled_bash = bundled_root.join("bin").join("bash.exe");
...
pub(crate) fn build_enriched_path() -> String {
    ...
    // Installer-bundled CLI runtime (offline package).
    if let Some(bundled_cli_dir) = bundled_cli_bin_dir() {
        paths.push(bundled_cli_dir.to_string_lossy().to_string());
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(git_dir) = bundled_git_root_dir() {
            let git_bin = git_dir.join("bin");
            if git_bin.exists() {
                paths.push(git_bin.to_string_lossy().to_string());
            }
...
async fn delete_session(session_id: String, session_path: String) -> Result<(), String> {
    ...
    if !session_path.is_empty() {
        ...
        if target.exists() {
            ...
            if !path_is_within(&canonical, &canonical_allowed) {
                return Err(format!(
            ...
            let sidecar_dir = canonical.with_extension("");
            if sidecar_dir.is_dir() {
                let canonical_sidecar = sidecar_dir
                    .canonicalize()
                    .map_err(|e| format!("Failed to canonicalize session folder: {}", e))?;
                if !path_is_within(&canonical_sidecar, &canonical_allowed) {
                    return Err(format!(
                        "Refusing to delete session folder outside ~/.claude/projects/: {:?}",
                        canonical_sidecar
                    ));
                }
                std::fs::remove_dir_all(&canonical_sidecar)
                    .map_err(|e| format!("Failed to delete session folder: {}", e))?;
            }
        }
    }
    Ok(())
...
async fn get_profile_stats() -> Result<Value, String> {
    ...
    fn profile_date_key_from_value(value: &Value) -> String {
        if let Some(timestamp) = value.get("timestamp").and_then(|v| v.as_str()) {
            ...
            if let Some(prefix) = timestamp.get(0..10) {
                return prefix.to_string();
            }
        }

        if let Some(timestamp_ms) = value.get("timestamp").and_then(|v| v.as_i64()) {
        ...
        for entry in entries.flatten() {
            ...
            for file in files.flatten() {
                ...
                for line in reader.lines().flatten() {
                    ...
                    let date = profile_date_key_from_value(&value);
...
async fn read_file_content(path: String) -> Result<String, String> {
    ...
    match ext.as_str() {
        "docx" => {
            if metadata.len() > MAX_OFFICE_REVIEW_BYTES {
                return Err("File too large (>120MB)".to_string());
            }
            let file = std::fs::File::open(&path).map_err(|e| format!("Cannot read file: {}", e))?;
            let mut zip = zip::ZipArchive::new(file).map_err(|e| format!("Cannot open docx: {}", e))?;
            let mut document = zip
                .by_name("word/document.xml")
                .map_err(|e| format!("Cannot read docx document: {}", e))?;
            let xml = read_xmlish_string(&mut document, "Cannot read docx text")?;
        ...
        "xlsx" => {
            ...
            let file = std::fs::File::open(&path).map_err(|e| format!("Cannot read file: {}", e))?;
            let mut zip = zip::ZipArchive::new(file).map_err(|e| format!("Cannot open xlsx: {}", e))?;
            extract_xlsx_text(&mut zip)
        }
        "pptx" => {
            if metadata.len() > MAX_OFFICE_REVIEW_BYTES {
                return Err("File too large (>120MB)".to_string());
            ...
            let file = std::fs::File::open(&path).map_err(|e| format!("Cannot read file: {}", e))?;
            let mut zip = zip::ZipArchive::new(file).map_err(|e| format!("Cannot open pptx: {}", e))?;
            extract_pptx_text(&mut zip)
        }
        "zip" => {
            if metadata.len() > MAX_ARCHIVE_PREVIEW_BYTES {
                return Err("Archive too large (>512MB)".to_string());
            }
        ...
        "tar" => {
            ...
            preview_tar_archive(&path)
        }
        "gz" => {
            if metadata.len() > MAX_ARCHIVE_PREVIEW_BYTES {
                return Err("Archive too large (>512MB)".to_string());
            }
            if lower_path.ends_with(".tar.gz") || lower_path.ends_with(".tgz") {
                preview_targz_archive(&path)
            } else {
                preview_gzip_stream(&path)
            }
        }
        "rar" => {
            if metadata.len() > MAX_ARCHIVE_PREVIEW_BYTES {
...
/// Limit: 120MB so larger local files can still be previewed in-app.
#[tauri::command]
...
#[tauri::command]
async fn export_session_text(path: String, output_path: String) -> Result<(), String> {
    use std::io::{BufRead, Write};
    let file = std::fs::File::open(&path).map_err(|e| format!("Failed to open session: {}", e))?;
    let reader = std::io::BufReader::new(file);
                ...
                match msg_type {
                    "user" | "human" => {
                        ...
                        if let Some(raw) = content.as_str() {
                            text_buf.push_str(raw.trim());
                        } else if let Some(arr) = content.as_array() {
                            for block in arr {
                                if let Some(raw) = block["text"].as_str() {
...
#[tauri::command]
async fn is_directory_path(path: String) -> Result<bool, String> {
    let metadata =
        std::fs::metadata(&path).map_err(|e| format!("Cannot read path metadata: {}", e))?;
    Ok(metadata.is_dir())
}
...
#[tauri::command]
async fn save_background_image(name: String, data: Vec<u8>) -> Result<String, String> {
    let dir = app_data_dir()?.join("backgrounds");
    std::fs::create_dir_all(&dir).map_err(|e| format!("Failed to create background dir: {}", e))?;
    ...
    std::fs::write(&path, &data).map_err(|e| format!("Failed to save background image: {}", e))?;
    Ok(path.to_string_lossy().to_string())
}
...
async fn run_git_command(cwd: String, args: Vec<String>) -> Result<String, String> {
    ...
    match subcmd {
        "add" => {
            for arg in &args[1..] {
                let lower = arg.to_lowercase();
                if matches!(lower.as_str(), "-p" | "--patch" | "-e" | "--edit" | "-i" | "--interactive") {
                    return Err(format!("Git add flag '{}' not allowed", arg));
                }
            }
        }
        "reset" => {
            for arg in &args[1..] {
                ...
                if lower.starts_with('-') {
                    return Err(format!("Git reset flag '{}' not allowed", arg));
                }
            }
            if args.len() >= 2 && args[1] != "HEAD" {
                return Err("Git reset only allows HEAD".to_string());
            }
        }
        "commit" => {
            ...
            for arg in &args[1..] {
                let lower = arg.to_lowercase();
                if lower == "-m" || lower == "--message" {
                    has_message = true;
                }
                if matches!(
                    lower.as_str(),
                    "--amend" | "--fixup" | "--squash" | "-c" | "-C" | "--reuse-message"
                        | "--reedit-message" | "--template" | "--interactive" | "--patch" | "--edit"
                ) {
                    return Err(format!("Git commit flag '{}' not allowed", arg));
                }
            }
            if !has_message {
                return Err("Git commit requires -m/--message".to_string());
            }
        }
        "branch" => {
            for arg in &args[1..] {
                let lower = arg.to_lowercase();
                if !(lower == "--show-current"
                    || lower == "--list"
                    || lower.starts_with("--format=")
                    || !lower.starts_with('-'))
                {
                    return Err(format!("Git branch flag '{}' not allowed", arg));
                }
            }
        }
        "switch" => {
            for arg in &args[1..] {
                let lower = arg.to_lowercase();
                if !(lower == "-c" || lower == "--create" || !lower.starts_with('-')) {
                    return Err(format!("Git switch flag '{}' not allowed", arg));
                }
            }
        }
        _ => {}
...
/// Uses the provider's fastest configured model (or Claude Sonnet by default).
...
async fn generate_session_title(
    ...
    let (provider_env, provider_keys_to_remove, model_name) = if let Some(ref pid) = provider_id {
        ...
        let provider = providers_file
            ...
            .find(|p| p.id == *pid)
            .ok_or_else(|| format!("Provider not found: {}", pid))?;
        (env, keys, resolve_translation_model(provider))
    } else {
        (
            HashMap::new(),
            vec![],
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            init_bundled_runtime_paths(&app.handle());
