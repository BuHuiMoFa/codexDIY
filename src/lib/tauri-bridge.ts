...
export const bridge = {
  ...
  exportSessionText: (path: string, outputPath: string) =>
    invoke<void>('export_session_text', { path, outputPath }),
  ...
  saveBackgroundImage: (name: string, data: number[]) =>
    invoke<string>('save_background_image', { name, data }),

  isDirectoryPath: (path: string) =>
    invoke<boolean>('is_directory_path', { path }),
  ...
  getGitRemoteUrl: (cwd: string, remoteName = 'origin') =>
    invoke<string | null>('get_git_remote_url', { cwd, remoteName }),
