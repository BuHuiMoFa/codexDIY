...
function App() {
  ...
  const workspaceDropRef = useRef<{ time: number; key: string }>({ time: 0, key: '' });
  ...
  useEffect(() => {
    const handler = () => {
      requestAppClose().catch(() => {});
    };
    window.addEventListener('tokenicode:request-app-close', handler as EventListener);
    return () => window.removeEventListener('tokenicode:request-app-close', handler as EventListener);
  }, []);
  ...
  useEffect(() => {
    ...
    import('@tauri-apps/api/window').then(({ getCurrentWindow }) => {
      getCurrentWindow().onDragDropEvent((event) => {
        ...
        if (now - workspaceDropRef.current.time < 500 && key === workspaceDropRef.current.key) return;
        workspaceDropRef.current = { time: now, key };
        ...
        void (async () => {
          ...
          for (const path of paths) {
            try {
              ...
              // Ignore invalid drop entries here; file handling is covered elsewhere.
            }
          }
          ...
          settings.addWorkspaces(workspacePaths);
          if (!settings.workingDirectory) {
            settings.setWorkingDirectory(workspacePaths[0]);
          }

          const count = workspacePaths.length;
          showToast(
            count === 1
              ? `已添加工作区：${workspacePaths[0]}`
              : `已添加 ${count} 个工作区`,
            'success',
          );
        })();
