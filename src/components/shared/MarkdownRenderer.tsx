...
/* ================================================================
   AsyncImage 閳?loads local files via Rust base64 bridge
   ================================================================ */
...
/* ================================================================
   CopyButton 閳?hover-reveal copy for code blocks
   ================================================================ */
...
/** Known code/config file extensions 閳?shared between wrapBareFilePaths and inline code detection. */
...
/** Detect file paths in inline code 閳?conservative regex to avoid false positives.
...
const STANDALONE_PATH_LINE_RE = /^(\s*(?:[-*+]\s+)?)(\*\*)?(.+?)(\*\*)?(\s*)$/;

function normalizeFileReferenceText(raw: string): string {
  ...
  normalized = normalized.replace(/(^|[^\\/])\\_(?=[^\\/]|$)/g, '$1_');
  normalized = normalized.replace(/^["'“”]+|["'“”]+$/g, '');

  return normalized.trim();
}

function getCandidateExtension(raw: string): string {
  const normalized = normalizeFileReferenceText(raw);
  return normalized.split('.').pop()?.toLowerCase() ?? '';
}

function isLikelyFileReference(raw: string): boolean {
  const normalized = normalizeFileReferenceText(raw);
  const ext = getCandidateExtension(normalized);
  if (!ext || !KNOWN_FILE_EXTENSIONS.has(ext)) return false;

  return (
    FILE_PATH_RE.test(normalized)
    || KNOWN_EXT_RE.test(normalized)
...
function wrapBareFilePaths(content: string): string {
  // Split by fenced code blocks (``` ... ```) 閳?don't touch code blocks
  const fenced = content.split(/(```[\s\S]*?```)/g);
  return fenced.map((part, i) => {
    if (i % 2 === 1) return part; // inside fenced code block
    // Split by inline code (` ... `) 閳?don't double-wrap
    const inlined = part.split(/(`[^`\n]+`)/g);
    return inlined.map((seg, j) => {
      if (j % 2 === 1) return seg; // inside inline code
      const standaloneWrapped = seg.split('\n').map((line) => {
        const match = line.match(STANDALONE_PATH_LINE_RE);
        if (!match) return line;
        const [, prefix, openStrong = '', body, closeStrong = '', suffix] = match;
        const candidate = body.trim();
        if (!isLikelyFileReference(candidate)) return line;
        const normalized = normalizeFileReferenceText(candidate);
        return `${prefix}${openStrong}\`${normalized}\`${closeStrong}${suffix}`;
      }).join('\n');
      // Also skip markdown link targets: [text](url)
      return standaloneWrapped.replace(BARE_PATH_RE, (match, prefix, path, offset, str) => {
        ...
        if (!isLikelyFileReference(path)) return match;
        return `${prefix}\`${normalizeFileReferenceText(path)}\``;
      });
    }).join('');
  }).join('');
}
...
/* ================================================================
   MarkdownRenderer 閳?shared markdown rendering with syntax highlighting
   ================================================================ */
interface Props {
  content: string;
  className?: string;
...
export const MarkdownRenderer = memo(function MarkdownRenderer({ content, className, basePath }: Props) {
  ...
  // Stable components object 閳?only recreated if `t` or resolveBase changes
  const components = useMemo(() => ({
    ...
    code: ({ children, className }: { children?: ReactNode; className?: string }) => {
      // Fenced code blocks (language-xxx) 閳?don't intercept, let <pre> handle them
      if (className) return <code className={className}>{children}</code>;

      const text = normalizeFileReferenceText(extractText(children).trim());
      const ext = text.split('.').pop()?.toLowerCase() ?? '';
      if (isLikelyFileReference(text) && KNOWN_FILE_EXTENSIONS.has(ext)) {
        ...
        return (
          <span
            className="inline-flex items-center gap-1 px-1.5 py-0.5 mx-0.5
              ...
              text-xs text-accent font-medium select-none
          ...
          >
            ...
            <button
              ...
              className="rounded-sm bg-accent/12 px-1 py-[1px] text-[10px] leading-none text-accent
            ...
            <button
              onClick={() => bridge.revealInFinder(resolved).catch(() => {})}
              className="rounded-sm border border-accent/20 px-1 py-[1px] text-[10px] leading-none text-accent
                hover:bg-accent/12 transition-smooth cursor-pointer"
              title={t('files.revealInFinder')}
            >
              文件夹
            </button>
          </span>
