...
export function AvatarCropModal({ imageFile, onSave, onCancel }: AvatarCropModalProps) {
  ...
  useEffect(() => {
    ...
    reader.onload = () => {
      if (!alive || typeof reader.result !== 'string') return;
      ...
      <div className="tokenicode-modal-surface bg-bg-card rounded-2xl shadow-2xl p-6 flex flex-col items-center gap-4 w-[320px]"
        ...
        >
          {imgSrc && (
            <img
              ...
              alt=""
              draggable={false}
              className="absolute left-0 top-0 pointer-events-none max-w-none"
              style={{
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="w-8 h-8 rounded-lg border border-border-subtle flex items-center justify-center
              text-text-muted hover:bg-bg-secondary transition-smooth text-sm font-bold disabled:opacity-40 disabled:cursor-not-allowed"
          >
        ...
        <div className="flex gap-3 w-full">
          ...
          <button
            ...
            disabled={!canInteract}
            className="flex-1 py-2 rounded-xl text-[13px] font-medium
              bg-accent text-text-inverse hover:bg-accent-hover transition-smooth disabled:opacity-50 disabled:cursor-not-allowed"
          >
