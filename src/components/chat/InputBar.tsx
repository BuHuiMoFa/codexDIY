...
const SPREADSHEET_CREATE_HINT = [
  '[TOKENICODE TASK HINT]',
  'This request is asking for spreadsheet generation, not just spreadsheet editing.',
  'If the user did not explicitly name an existing workbook to modify, create a new .xlsx output file in the working directory.',
  'If the prompt points to a folder, inspect that folder and generate the workbook from those source files.',
  'Do not default to only updating an existing workbook unless the user clearly asked to edit that workbook.',
  'In the final answer, clearly list the created workbook path(s).',
].join('\n');
...
function shouldInjectSpreadsheetCreateHint(text: string): boolean {
  ...
  const explicitWorkbookPatterns = [
    /[A-Za-z]:[^\n]*\.xlsx?\b/i,
    /(^|[\s`"'])[^ \n`"']+\.xlsx?\b/i,
  ];

  const asksForSpreadsheet = hasMatch(normalized, spreadsheetPatterns);
  const asksToCreate = hasMatch(normalized, createPatterns);
  const asksToEdit = hasMatch(normalized, editPatterns);
  const mentionsWorkbook = hasMatch(normalized, explicitWorkbookPatterns);

  if (!asksForSpreadsheet) return false;
  if (!asksToCreate) return false;
  if (asksToEdit && mentionsWorkbook) return false;
  return true;
}
...
function ThinkLevelSelector({ disabled = false }: { disabled?: boolean }) {
  ...
  const current = THINK_LEVELS.find((l) => l.id === thinkingLevel) || THINK_LEVELS[1];
...
export function InputBar() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('tokenicode:focus-input', handler);
    return () => window.removeEventListener('tokenicode:focus-input', handler);
  }, []);

  useEffect(() => {
    const handler = (event: Event) => {
      const detail = (event as CustomEvent<{ text?: string; messageId?: string }>).detail;
      if (!detail?.text || !detail?.messageId) return;
      inlineEditSubmitRef.current = { messageId: detail.messageId };
      setInputSync(detail.text);
      requestAnimationFrame(() => {
        handleSubmitRef.current();
      });
    };
    window.addEventListener('tokenicode:submit-edited-message', handler);
    return () => window.removeEventListener('tokenicode:submit-edited-message', handler);
  }, [setInputSync]);
  ...
  const inlineEditSubmitRef = useRef<{ messageId: string } | null>(null);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (shouldInjectSpreadsheetCreateHint(rawInput)) {
      text = `${text}\n\n${SPREADSHEET_CREATE_HINT}`;
    }

    setInputSync('');

    const inlineEditTarget = inlineEditSubmitRef.current;
    ...
    if (silentRestartRef.current) {
      ...
      inlineEditSubmitRef.current = null;
      const existingMessage = useChatStore.getState().getTab(tabId)?.messages
        .find((msg) => msg.id === inlineEditTarget.messageId);
      useChatStore.getState().updateMessage(tabId, inlineEditTarget.messageId, {
    ...
    useChatStore.getState().pushProcessEvent(tabId, {
      kind: 'status',
      label: 'Turn started',
      detail: rawInput.trim().slice(0, 120),
      at: turnStartedAt,
    });
  ...
  const handleStderrLine = useCallback((line: string, sid: string) => {
    if (sid) {
      ...
      if (ownerTabId && ownerTabId !== activeTabId) {
        ...
        if (clean) {
          useChatStore.getState().setSessionMeta(ownerTabId, { lastStderrLine: clean });
          useChatStore.getState().pushProcessEvent(ownerTabId, {
            kind: 'stderr',
            label: 'stderr',
    ...
    if (clean && !/^\s*$/.test(clean)) {
      ...
      if (stderrTabId) {
        useChatStore.getState().setSessionMeta(stderrTabId, { lastStderrLine: clean });
        useChatStore.getState().pushProcessEvent(stderrTabId, {
          kind: 'stderr',
          label: 'stderr',
          detail: clean.slice(0, 200),
        });
