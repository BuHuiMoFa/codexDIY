...
import { useSettingsStore, normalizeWorkspacePath } from '../../stores/settingsStore';
...
export function ConversationList() {
  ...
  const savedWorkspaces = useSettingsStore((s) => s.workspaces);
  ...
  // Group by saved workspaces first, then by discovered project history
  const workspaceGroups = useMemo<WorkspaceGroupEntry[]>(() => {
    const savedPaths = savedWorkspaces
      .map((workspace) => normalizeWorkspacePath(workspace.path))
      .filter(Boolean);
    const map = new Map<string, SessionListItem[]>();

    for (const session of filtered) {
      const key = getSessionWorkspacePath(session);
      if (!key) continue;
    ...
    const entries: WorkspaceGroupEntry[] = savedPaths.map((workspacePath) => ({
      key: workspacePath,
      projectPath: workspacePath,
      sessions: map.get(workspacePath) || [],
      isSavedWorkspace: true,
    }));

    for (const workspacePath of savedPaths) {
    ...
    const remaining = Array.from(map.entries())
      ...
      .sort((a, b) => {
        const ta = a.sessions[0]?.modifiedAt || 0;
        const tb = b.sessions[0]?.modifiedAt || 0;
        return tb - ta;
      });

    return [...entries, ...remaining];
  }, [filtered, savedWorkspaces]);
  ...
  const isExpanded = useCallback((key: string) => {
    ...
    if (!raw) return false;
    const selectedKey = getSessionWorkspacePath(raw);
  ...
  const executeDelete = useCallback(async (sessionId: string, sessionPath: string) => {
    try {
      ...
      persistPinned(new Set([...pinnedSessions].filter((id) => id !== sessionId)));
      persistArchived(new Set([...archivedSessions].filter((id) => id !== sessionId)));
      useChatStore.getState().removeFromCache(sessionId);
      await fetchSessions();
    } catch (err) {
      console.error('Failed to delete session:', err);
      showToast(`${t('conv.deleteFailed')}: ${String(err)}`, 'error');
    }
  }, [
    archivedSessions,
    fetchSessions,
  ...
  const confirmDeleteAll = useCallback(async () => {
    ...
    const allSessions = useSessionStore.getState().sessions;
    const projectSessions = allSessions.filter((s) => {
      return getSessionWorkspacePath(s) === deleteAllTarget.projectKey;
    });
  ...
  const handleProjectContextMenu = useCallback((e: React.MouseEvent, project: string) => {
    e.preventDefault();
    e.stopPropagation();
    const normalized = normalizeWorkspacePath(project);
    const isSavedWorkspace = savedWorkspaces.some(
      (workspace) => normalizeWorkspacePath(workspace.path) === normalized,
    );
    setProjectMenu({ x: e.clientX, y: e.clientY, project, isSavedWorkspace });
  }, [savedWorkspaces]);

  const handleRemoveWorkspace = useCallback((project: string) => {
    setRemoveWorkspaceTarget(project);
  }, []);

  const confirmRemoveWorkspace = useCallback(() => {
    if (!removeWorkspaceTarget) return;
    removeWorkspace(removeWorkspaceTarget);
    ...
    <div className="flex flex-col gap-1 px-3">
      ...
      {projectMenu && (
        <ProjectContextMenu
          ...
          isSavedWorkspace={projectMenu.isSavedWorkspace}
          onNewSession={handleNewSessionInProject}
      ...
      {removeWorkspaceTarget && (
        <ConfirmDialog
          open={true}
          title="移出工作区"
          message="确定要把这个文件夹从工作区列表中移除吗？"
          detail="这不会删除文件夹和里面的历史会话，只会从左侧工作区列表移除。"
          confirmLabel="移除"
          onConfirm={confirmRemoveWorkspace}
          onCancel={() => setRemoveWorkspaceTarget(null)}
        />
      )}
    </div>
  );
