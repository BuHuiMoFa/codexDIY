...
export function ExportMenu({ sessionPath }: Props) {
  ...
  const handleExport = async (format: 'markdown' | 'text' | 'json') => {
    ...
    const filterName = format === 'markdown' ? 'Markdown' : format === 'text' ? 'Text' : 'JSON';
    const timestamp = new Date().toISOString().slice(0, 10);
    const suffix = format === 'text' ? '_conversation' : '';
    ...
    try {
      if (format === 'markdown') {
        await bridge.exportSessionMarkdown(sessionPath, outputPath, false);
      } else if (format === 'text') {
        await bridge.exportSessionText(sessionPath, outputPath);
      } else {
