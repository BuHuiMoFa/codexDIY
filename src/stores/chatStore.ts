...
export interface ProcessEvent {
  id: string;
  at: number;
  kind: 'status' | 'tool' | 'stderr' | 'stream' | 'result' | 'warning';
  label: string;
  detail?: string;
}
...
export interface SessionMeta {
  ...
  /** Recent runtime activity for the live process panel */
  processEvents?: ProcessEvent[];
  /** Last visible process event timestamp */
  lastEventAt?: number;
  /** Last visible process event summary */
  lastEventLabel?: string;
  /** Latest stderr line captured for live diagnostics */
...
interface ChatState {
  ...
  pushProcessEvent: (tabId: string, event: Omit<ProcessEvent, 'id' | 'at'> & Partial<Pick<ProcessEvent, 'id' | 'at'>>) => void;
  ...
  truncateAfterMessage: (tabId: string, messageId: string) => void;
...
export const useChatStore = create<ChatState>()((set, get) => ({
  ...
  pushProcessEvent: (tabId, event) =>
    set((state) => {
      const result = updateTab(state.tabs, tabId, (tab) => {
        const nextAt = event.at ?? Date.now();
        const nextEvent: ProcessEvent = {
          id: event.id ?? generateMessageId(),
          at: nextAt,
          kind: event.kind,
          label: event.label,
          detail: event.detail,
        };
        const current = tab.sessionMeta.processEvents ?? [];
        const last = current[current.length - 1];
        const merged = last
          && last.kind === nextEvent.kind
          && last.label === nextEvent.label
          && last.detail === nextEvent.detail
          ? [...current.slice(0, -1), { ...last, at: nextAt }]
          : [...current, nextEvent].slice(-24);
        const summary = nextEvent.detail ? `${nextEvent.label}: ${nextEvent.detail}` : nextEvent.label;
        return {
          ...tab,
          sessionMeta: {
            ...tab.sessionMeta,
  ...
  truncateAfterMessage: (tabId, messageId) => {
    ...
    set((state) => {
      const result = updateTab(state.tabs, tabId, (tab) => {
        ...
        return {
          ...tab,
          messages: tab.messages.slice(0, idx + 1),
          isStreaming: false,
          partialText: '',
          partialThinking: '',
          pendingUserMessages: [],
