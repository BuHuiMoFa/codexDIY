...
interface SessionState {
  ...
  /** Remove a persisted session from local state after deletion */
...
export const useSessionStore = create<SessionState>()((set, get) => ({
  ...
  removeSession: (sessionId) => set((state) => {
    ...
    for (const [stdinId, tabId] of Object.entries(stdinToTab)) {
      if (tabId === sessionId) delete stdinToTab[stdinId];
    }
    ...
    if (customPreviews[sessionId]) {
      delete customPreviews[sessionId];
    ...
    return {
      ...
      previousSessionId: state.previousSessionId === sessionId ? null : state.previousSessionId,
      runningSessions,
      stdinToTab,
