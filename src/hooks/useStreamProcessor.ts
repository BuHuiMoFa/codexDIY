...
function _pushProcessEvent(
  tabId: string,
  kind: import('../stores/chatStore').ProcessEvent['kind'],
  label: string,
  detail?: string,
) {
  useChatStore.getState().pushProcessEvent(tabId, {
    kind,
    label,
...
function _formatToolCounts(names: string[]): string {
...
function _collectToolNames(input: unknown): string[] {
  if (!input) return [];
  if (Array.isArray(input)) {
    return input.flatMap((item) => _collectToolNames(item));
  }
  ...
  if (typeof input === 'object') {
    ...
    return [
      ..._collectToolNames(obj.name),
      ..._collectToolNames(obj.tool_name),
      ..._collectToolNames(obj.tool),
      ..._collectToolNames(obj.tools),
      ..._collectToolNames(obj.tool_uses),
      ..._collectToolNames(obj.calls),
    ];
  }
  return [];
}

function _extractToolSummary(msg: any): { label: string; detail?: string; toolName?: string } {
  const names = _collectToolNames(
    msg?.tool_uses
    ?? msg?.tools
    ?? msg?.calls
    ?? msg?.summary?.tool_uses
    ?? msg?.summary?.tools
    ?? msg?.summary?.calls,
  );
  const formattedNames = _formatToolCounts(names);
  const textSummary = [
    msg?.summary_text,
  ...
  if (formattedNames) {
    ...
    return {
      label: count > 0 ? `${count} tool calls` : 'Tool summary',
      detail: formattedNames,
      toolName: names[0],
    };
  }
  ...
  const fallbackName = String(msg?.tool_name || msg?.name || '').trim();
  return {
    label: 'Tool activity',
    detail: fallbackName || undefined,
    toolName: fallbackName || undefined,
  };
}
...
export function useStreamProcessor(config: StreamProcessorConfig) {
    ...
    switch (msg.type) {
      case 'tokenicode_permission_request': {
        ...
        if (msg.tool_name === 'ExitPlanMode') {
          ...
          _pushProcessEvent(tabId, 'warning', 'Awaiting review', msg.tool_name);
          return;
        }
        ...
        if (msg.tool_name === 'AskUserQuestion') {
          ...
          store.setActivityStatus(tabId, { phase: 'awaiting' });
          _pushProcessEvent(tabId, 'warning', 'Awaiting input', 'AskUserQuestion');
          return;
        }
        ...
        store.setActivityStatus(tabId, { phase: 'awaiting' });
        _pushProcessEvent(tabId, 'warning', 'Awaiting permission', msg.tool_name);
        break;
      }
      case 'stream_event': {
        const evt = msg.event;
        ...
        if (evt.type === 'content_block_delta' && evt.delta?.type === 'text_delta') {
          ...
          if (text) {
            ...
            if (store.getTab(tabId)?.activityStatus.phase !== 'writing') {
              _pushProcessEvent(tabId, 'stream', 'Writing');
            }
          }
        }
        if (evt.type === 'content_block_delta' && evt.delta?.type === 'thinking_delta') {
          if (store.getTab(tabId)?.activityStatus.phase !== 'thinking') {
            _pushProcessEvent(tabId, 'stream', 'Thinking');
          }
        }
        if (evt.type === 'content_block_start' && evt.content_block?.type === 'tool_use') {
          _pushProcessEvent(tabId, 'tool', 'Tool', evt.content_block.name);
        }
        ...
        if (evt.type === 'message_start' && evt.message?.usage?.input_tokens) {
          _pushProcessEvent(tabId, 'status', 'Message started');
          const bgTab = store.getTab(tabId);
        ...
        if (evt.type === 'message_stop') {
          _pushProcessEvent(tabId, 'result', 'Message finished');
        }
        break;
      }
      case 'tool_use_summary': {
        const summary = _extractToolSummary(msg);
        store.setActivityStatus(tabId, {
          phase: 'tool',
          toolName: summary.toolName || 'summary',
        });
        _pushProcessEvent(tabId, 'tool', summary.label, summary.detail);
        break;
      }
      case 'assistant': {
  ...
  const handleStreamMessage = useCallback((msg: any) => {
    ...
    let tabId: string = ownerTabId || activeTabId || '';
    if (!tabId) return;
    ...
    if (msg.type === 'tokenicode_permission_request') {
      ...
      if (msg.tool_name === 'ExitPlanMode') {
        ...
        if (planReview) {
          ...
          _pushProcessEvent(tabId, 'warning', 'Awaiting review', 'ExitPlanMode');
        }
        return;
      }
    ...
    const pushProcessEvent = (
      kind: import('../stores/chatStore').ProcessEvent['kind'],
      label: string,
      detail?: string,
    ) => _pushProcessEvent(tabId, kind, label, detail);
    ...
    if (cliSessionId) {
      ...
      if (currentId !== cliSessionId) {
        ...
        if (tabId.startsWith('draft_')) {
          ...
          // Critical: continue routing this live turn into the promoted tab.
          // Without rebinding, subsequent assistant/result/process_exit updates
          // still target the deleted draft tab and only appear after reload.
          tabId = cliSessionId;
        }
    ...
    switch (msg.type) {
      ...
      case 'stream_event': {
        ...
        if (evt.type === 'content_block_delta' && evt.delta?.type === 'text_delta') {
          ...
          if (text && msgStdinId) {
            ...
            if (useChatStore.getState().getTab(tabId)?.activityStatus.phase !== 'writing') {
              pushProcessEvent('stream', 'Writing');
            }
          }
          ...
          if (thinkingText && msgStdinId) {
            ...
            if (useChatStore.getState().getTab(tabId)?.activityStatus.phase !== 'thinking') {
              pushProcessEvent('stream', 'Thinking');
            }
          } else {
        ...
        if (evt.type === 'content_block_start'
          ...
          pushProcessEvent('tool', 'Tool', evt.content_block.name);
        ...
        if (evt.type === 'message_start' && evt.message?.usage?.input_tokens) {
          pushProcessEvent('status', 'Message started');
        ...
        // Some providers / CLI flows reliably emit message_stop even when a final
        // result event is delayed or omitted. Finalize the streamed text here so
        // the UI does not remain stuck in "thinking" after the visible answer is done.
        if (evt.type === 'content_block_stop' && msgStdinId) {
          flushStreamBuffer(msgStdinId);
        }
        if (evt.type === 'message_stop') {
          pushProcessEvent('result', 'Message finished');
          if (msgStdinId) {
            flushStreamBuffer(msgStdinId);
          }
          ...
          const hasPendingInteraction = currentMessages.some(
          ...
          if (!hasPendingInteraction) {
            const streamedThinking = currentTab?.partialThinking?.trimEnd() || '';
            if (streamedThinking) {
              let duplicateThinking = false;
              for (let i = currentMessages.length - 1; i >= 0; i--) {
                const existing = currentMessages[i];
                if (existing.role === 'user') break;
                if (existing.role === 'assistant'
                    && existing.type === 'thinking'
                    && existing.content === streamedThinking) {
                  duplicateThinking = true;
                  break;
                }
              }
            ...
            const streamedText = currentTab?.partialText?.trimEnd() || '';
            if (streamedText) {
            ...
            setActivityStatus({ phase: 'completed' });
            agentActions.completeAgent(agentId, 'completed');
            setSessionMeta({ lastProgressAt: undefined });
          }
        }
        break;
      }
      ...
      case 'assistant': {
        ...
        for (let blockIdx = 0; blockIdx < content.length; blockIdx++) {
          ...
          if (block.type === 'text') {
            ...
            {
              const currentMessages = useChatStore.getState().getTab(tabId)?.messages ?? [];
              let duplicateInCurrentTurn = false;
              for (let i = currentMessages.length - 1; i >= 0; i--) {
                const existing = currentMessages[i];
                if (existing.role === 'user') break;
                if (existing.role === 'assistant'
                  ...
                  duplicateInCurrentTurn = true;
                  break;
                }
              }
              if (duplicateInCurrentTurn) continue;
            }
            pushProcessEvent('stream', 'Writing');
            ...
            pushProcessEvent('tool', 'Tool', block.name);
            ...
            if (block.name === 'AskUserQuestion') {
              ...
              setActivityStatus({ phase: 'awaiting' });
              pushProcessEvent('warning', 'Awaiting input', 'AskUserQuestion');
            } else if (block.name === 'TodoWrite' && block.input?.todos) {
              addMessage({
                id: block.id || generateMessageId(),
                role: 'assistant',
              ...
              if (getEffectiveMode(useChatStore.getState().getTab(tabId)?.sessionMeta) === 'plan') {
                ...
                if (!(toolAlreadyExisted && existingReview)) {
                  ...
                  setActivityStatus({ phase: 'awaiting' });
                  pushProcessEvent('warning', 'Awaiting review', 'ExitPlanMode');
                }
              }
            } else {
              addMessage({
      ...
      case 'tool_result': {
        ...
        pushProcessEvent('result', 'Tool result', msg.tool_name);
        addMessage({
          id: msg.uuid || generateMessageId(),
          role: 'assistant',
          type: 'tool_result',
      ...
      case 'tool_use_summary': {
        ...
        pushProcessEvent('tool', summary.label, summary.detail);
        break;
      }
      ...
      case 'result': {
        ...
        if (msg.parent_tool_use_id) {
          ...
          pushProcessEvent('result', 'Sub-agent result', msg.subtype === 'success' ? 'completed' : 'error');
          break;
        }
        ...
        if (msg.subtype !== 'success') {
          pushProcessEvent('warning', 'Result error', typeof msg.result === 'string' ? msg.result : msg.error || msg.content);
          const meta = useChatStore.getState().getTab(tabId)?.sessionMeta ?? {};
        ...
        pushProcessEvent(
          'result',
          msg.subtype === 'success' ? 'Turn completed' : 'Turn failed',
          typeof resultDisplayText === 'string' && resultDisplayText
            ? resultDisplayText
            : undefined,
        );
        {
      ...
      case 'process_exit': {
        ...
        const exitTabBeforeClear = useChatStore.getState().getTab(tabId);
        const exitMessagesBeforeClear = exitTabBeforeClear?.messages ?? [];
        const exitPartialThinking = exitTabBeforeClear?.partialThinking?.trimEnd() || '';
        const exitPartialText = exitTabBeforeClear?.partialText?.trimEnd() || '';
        const hasPendingInteraction = exitMessagesBeforeClear.some(
          (m: ChatMessage) => ['permission', 'question', 'plan_review'].includes(m.type)
            && !m.resolved
            && m.interactionState !== 'resolved'
            && m.interactionState !== 'sending',
        );

        if (!hasPendingInteraction && exitPartialThinking) {
          const duplicateThinking = exitMessagesBeforeClear.some(
            (m: ChatMessage) => m.role === 'assistant' && m.type === 'thinking' && m.content === exitPartialThinking,
          );
          if (!duplicateThinking) {
            addMessage({
              id: generateMessageId(),
        ...
        if (!hasPendingInteraction && exitPartialText) {
          const duplicateText = exitMessagesBeforeClear.some(
            (m: ChatMessage) => m.role === 'assistant' && m.type === 'text' && m.content === exitPartialText,
          );
          if (!duplicateText) {
            addMessage({
              id: generateMessageId(),
        ...
        pushProcessEvent('warning', 'Process exited');
        ...
        if (exitStatus === 'running') {
          const hasAssistantReply = exitMsgs.some(
            (m: ChatMessage) => m.role === 'assistant' && (m.type === 'text' || m.type === 'tool_use' || m.type === 'thinking'),
          );
          if (!hasAssistantReply) {
            if (lastStderrRef.current) {
              pushProcessEvent('stderr', 'stderr', lastStderrRef.current);
        ...
        const finalMessages = useChatStore.getState().getTab(tabId)?.messages ?? [];
        const hasAssistantReplyAfterExit = finalMessages.some(
        ...
        if (exitStatus === 'running' && hasAssistantReplyAfterExit && !hasPendingInteraction) {
          pushProcessEvent('result', 'Turn completed', 'Process exited after visible response');
          setSessionStatus('completed');
          setActivityStatus({ phase: 'completed' });
        } else {
